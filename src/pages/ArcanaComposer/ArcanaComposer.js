...
class ArcanaComposer extends React.Component {
  ...
  componentDidMount() {
    ...
    const arcanaID = params['arcana']

    if (arcanaID) {
      this.setState({
        uid: arcanaID
      }, () => {
        this.observeArcana(arcanaID)      
      })
    }
    else {
  ...
  arcanaForState() {
    ...
    delete arcana.loaded

    if (!arcana.uid) {
      if (!arcana.numberOfViews) {
        arcana.numberOfViews = 0
      }
  
      if (!arcana.numberOfLikes) {
        arcana.numberOfLikes = 0
      }
